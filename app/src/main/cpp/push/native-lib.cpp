...
Java_com_season_example_learnffmpeg_pusher_PusherNative_setVideoOptions(
...
void *publisher(void *args) {
    ...
    LOGI("Native found Java Publisher Object:%d", jPublisherObj ? 1 : 0);
    ...
    do {
        ...
        while (publishing) {
            ...
            if (vec.size() > 0) {
                for (std::vector<RTMPPacket *>::iterator it = vec.begin();
                     it != vec.end();) {
    ...
    free(rtmp_path);
    rtmp_path = NULL;
    std::vector<RTMPPacket *>::iterator iter = vec.begin();
    for (; iter != vec.end();) {
...
Java_com_season_example_learnffmpeg_pusher_PusherNative_startPush(
    ...
    pthread_create(&publisher_tid, NULL, publisher, NULL);
...
extern "C"
JNIEXPORT void JNICALL
Java_com_season_example_learnffmpeg_pusher_PusherNative_setAudioOptions(
...
Java_com_season_example_learnffmpeg_pusher_PusherNative_fireAudio(JNIEnv *env, jobject instance,
    ...
    jbyte *b_buffer = env->GetByteArrayElements(buffer_, NULL);
    ...
    if (bitbuf)
        free(bitbuf);
    env->ReleaseByteArrayElements(buffer_, b_buffer, 0);
}
...
extern "C"
JNIEXPORT void JNICALL
Java_com_season_example_learnffmpeg_pusher_PusherNative_release(JNIEnv *env, jobject thiz) {

    Java_com_season_example_learnffmpeg_pusher_PusherNative_stopPush(env, thiz);
    if (audioEncHandle) {
